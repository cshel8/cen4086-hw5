...
export class SupportStore {
  ...
  async createSupportTicket({ orderId, issue, confirmed }) {
    ...
    */
    ...
    const ticketId = `TKT-${randomUUID().replace(/-/g, '').slice(0, 8).toUpperCase()}`;
    const createdAt = new Date().toISOString();
    const ticket = { ticketId, orderId, issue, status: 'open', createdAt };

    const tickets = await readJson(this.ticketsFile);
    tickets.push(ticket);
    await writeFile(this.ticketsFile, JSON.stringify(tickets, null, 2));

    const auditEvent = { event: 'ticket_created', ticketId, orderId, createdAt };
